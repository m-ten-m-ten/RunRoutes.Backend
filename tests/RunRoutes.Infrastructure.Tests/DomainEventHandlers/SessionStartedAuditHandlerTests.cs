...
[Collection("Database")]
public class SessionStartedAuditHandlerTests(PostgresContainerFixture fixture)
{
    ...
    public async Task HandleAsync_AuditLogが正しく保存される()
    {
        ...
        var domainEvent = new SessionStartedEvent(sessionId, userId, occurredAt);
        ...
        {
            var handler = new SessionStartedAuditHandler(db, currentUserMock.Object);
        ...
        await using (var db = _fixture.CreateDbContext())
        {
            ...
            Assert.Equal(nameof(SessionStartedEvent), saved.EventType);
    ...
    public async Task HandleAsync_ActorIdがnullでも保存できる()
    {
        ...
        var sessionId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var domainEvent = new SessionStartedEvent(sessionId, userId, DateTime.UtcNow);
